...
class CGrabbarBackend {
  public:
    ...
    void                setSnapZone(const std::string& zone) { m_snapZone = zone; }
  ...
  private:
    ...
    TabStore                                        m_tabs;
    bool                                            m_dragging = false; // a title/tab drag is in progress (snap.status)
    std::string                                     m_snapZone = "none";
    ...
    void                                   broadcastTabs();
    std::string                            snapStatusJson() const;
