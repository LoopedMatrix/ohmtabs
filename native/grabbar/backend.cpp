...
std::string CGrabbarBackend::snapStatusJson() const {
    return std::format("{{\"dragging\":{},\"zone\":\"{}\",\"glow\":false}}", m_dragging, jsonEscape(m_snapZone));
}
...
void CGrabbarBackend::handleLine(SGrabbarClient* c, const std::string& line) {
    ...
    if (type == "snap") {
        send(c, "snapStatus", {{"json", snapStatusJson()}});
        return;
    }
