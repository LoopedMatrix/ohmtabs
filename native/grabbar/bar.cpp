...
void CGrabbarDeco::snapToZone() {
    ...
    // Report the resolved zone for snap.status (basic version: the zone the
    // release decided; the visual glow is another layer's job).
    std::string zone = "none";
    if (cornerTL)      zone = "corner-tl";
    else if (cornerTR) zone = "corner-tr";
    else if (cornerBL) zone = "corner-bl";
    else if (cornerBR) zone = "corner-br";
    else if (nearLeft) zone = "left";
    else if (nearRight) zone = "right";
    else if (nearTop)  zone = "top";
    g_pBackend->setSnapZone(zone);
